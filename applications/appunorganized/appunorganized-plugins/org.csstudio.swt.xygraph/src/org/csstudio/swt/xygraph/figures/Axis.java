...
public class Axis extends LinearScale {
    ...
    private boolean limits_from_pv = false;
    ...
    protected void layout() {
        ...
        if (isHorizontal() && getTickLablesSide() == LabelSide.Primary) {
            ...
            mousePositionLabel.setBounds(new Rectangle(
    ...
    /**
     * Perform an auto-scale: Axis limits are set to the value range of the traces
     * on this axis. Includes some optimization: Axis range is set a little wider
     * than exact trace data range. When auto-scale would only perform a minor axis
     * adjustment, axis is left unchanged.
     *
     * @param force
     ...
     * @param
     */
    public void setLimitsFromPV(final boolean limits_from_pv) {
        this.limits_from_pv = limits_from_pv;
    }
     ...
     * percentage of the plot area for the threshold when autoScale is performed.The
     * autoScale will performed only if the spare space exceeds this threshold. So
     * it can reduce the CPU usage by increasing the threshold.
     *
     ...
     *
     * @param onPrimarySide
     *            set true if the axis on primary side(Bottom/Left); false if it is
     *            not on the primary side of xy graph(Top/Right).
     */
    ...
    /**
     * Listener to mouse events, performs panning and some zooms Is very similar to
     * the PlotMouseListener, but unclear how easy/useful it would be to base them
     * on the same code.
     */
